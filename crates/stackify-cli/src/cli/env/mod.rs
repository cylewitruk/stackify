...
fn exec_list(ctx: &CliContext, _args: args::ListArgs) -> Result<()> {
    ...
    if environments.is_empty() {
        ...
        println!("To create a new environment, use the {} command.", style("stackify env create").white().bold());
        return Ok(());
    }
...
fn exec_create(ctx: &CliContext, args: args::CreateArgs) -> Result<()> {
    ...
    let env = ctx.db.create_environment(env_name.as_ref(), args.bitcoin_block_speed)?;
...
fn exec_start(ctx: &CliContext, args: args::StartArgs) -> Result<()> {
    ...
    PbWrapper::new_spinner("Create environment container")
        .exec(|pb| {
            ...
            if !env_container.warnings.is_empty() {
                ...
                env_container.warnings.iter().for_each(|w| println!("  - {}", w));
            }
            Ok(())
        })?;
    ...
    PbWrapper::new_spinner("Creating environment network")
        .exec(|pb| {
...
fn exec_stop(ctx: &CliContext, args: args::StopArgs) -> Result<()> {
    ...
    if containers.is_empty() {
        ...
        println!("To start the environment, use the {} command.", style("stackify env start").white().bold());
        return Ok(());
    }
